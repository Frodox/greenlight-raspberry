...
class Counter:
    ...
    # 0x06 == ACK - approve
    # 0x01 == SOH - start header
    # 0x02 == STX - start frame
    # 0x03 == ETX - end frame
    ...
    def __init__(self, port, debug = False, boud = 9600):
        ...
        self.debug = debug;
        # 8-n => 7-1
        self.ser = serial.Serial(self.port, boud, serial.EIGHTBITS, serial.PARITY_NONE, serial.STOPBITS_ONE, timeout = 0.5);
    ...
    def init(self):
        ...
        if (init):
            if "ERR" in init:
                self._init = False
                raise IOError("Failed to init serial connection {}".format(init))
...
#            print("Init = {}".format(init))
    ...
    def cmd(self, cmd, raw=False):
        ...
        if raw:
            res = answer[answer.find("(")+1:answer.find(")")]
        else:
    ...
    def getValue(self, answer):
        value = list(map(float, re.findall('(\d+.\d+)', answer)))
...
if __name__ == '__main__':
    try:
        en = Counter('/dev/ttyUSB0', False);
        #print en.get()
        en.mode('w')
    except serial.serialutil.SerialException as e:
        raise(e)

    TOTAL = en.cmd('ET0PE(1)')
    TIME = en.cmd('TIME_()', raw=True)
    DATE = en.cmd('DATE_()', raw=True)

    print('TOTAL = {} Квт/ч'.format(TOTAL))
    print('T1 = {0[0]} Квт/ч\r\nT2 = {0[1]} Квт/ч'.format(en.cmd('ET0PE(2,2)')))
    #print '{0[0]} /  {0[1]} Квт/ч'.format(en.cmd('ET0PE(1,1)'));
    print("Напряжение (U) = {:.2f} В".format(en.cmd('VOLTA()')))
    print("Ток = {:.2f} A".format(en.cmd('CURRE()')))
    print("Мощность (P) = {:.2f} КВт".format(en.cmd('POWEP()') * 1000))
    print("Время = {} ".format(TIME))
